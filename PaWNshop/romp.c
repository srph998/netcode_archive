/*
        ...
        i'm sorry but i'm not writing this as a cross-platform script,
...
// const
#define PING_PKT_S 32
#define PORT_NO 0
#define PING_SLEEP_RATE 1000000 //microsec
...
char *reverse_dns_lookup(char *ip_addr) {
        ...
        if (getnameinfo((struct sockaddr *)&temp_addr, len, buf, sizeof(buf), NULL, 0, NI_NAMEREQD)) {
                printf("[ERROR] Reverse DNS Lookup of Hostname\n");
                return NULL;
        }
...
void send_ping(int ping_sockfd, struct sockaddr_in *ping_addr, char *ping_dom, char *ping_ip, char *rev_host) {
        int ttl_val = 64, msg_count = 0, i, addr_len, flag = 1, msg_recieved_count = 0;
        char rbuffer[128];
        ...
        if (recvfrom(ping_sockfd, rbuffer, sizeof(rbuffer), 0, (struct sockaddr*)&r_addr, &addr_len) <= 0 && msg_count > 1) {
                ...
                if (flag) {
                        ...
                        if (!(recv_hdr->type == 0 && recv_hdr->code == 0)) {
                                printf("[ERROR] packet ICMP type %d code %d\n", recv_hdr->type, recv_hdr->code);
                        } else {
        ...
        printf("%d packets sent, %d packets recieved, %f%%packet loss. Total time: %Lf ms.n\n\n", msg_count, msg_recieved_count, ((msg_count - msg_recieved_count) / (double)msg_count) * 100.0, total_msec);
        }
        ...
        int main(int argc, char *argv[]) {
                ...
                if (sockfd < 0) {
                        printf("Socket file descriptor not recieved\n");
                        return 0;
                } else {
                        printf("Socket file descriptor %d recieved\n", sockfd);
                }
