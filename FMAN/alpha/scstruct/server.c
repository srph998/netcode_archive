...
int main(int argc, char *argv[] ) {
	const uint16_t port_number = 1337;
	...
	if (pid == 0) {
		...
		printf("Connected with '%d', process is %d.\nWaiting...\n", client_fd, getpid());
		...
		while (1) {
			...
			if (strlen(buffer) == 0) {
				...
				if (dif > 5.0) {
					printf("Process %d: ", getpid());
					close(client_fd);
					printf("Connection timed out after %.31f seconds.", dif);
			...
			printf("Process %d: ", getpid());
			printf("Recieved '%s'. Processing... ", buffer);
